...
export function testValidFeedbackSubmission<PageClass extends AnalyticsPage>(pageClass: new () => PageClass, chartIds) {
  chartIds.forEach(chartId => {
    ...
    page.getGraphFeedback(chartId).click()
    page.getGraphFeedbackForm(chartId).find('[name=chartUseful][value=yes]').click()
    page.getGraphFeedbackForm(chartId).submit()
...
export function testInvalidFeedbackSubmission<PageClass extends AnalyticsPage>(
  ...
  chartIds.forEach(chartId => {
    ...
    page.getGraphFeedback(chartId).click()
    page.getGraphFeedbackForm(chartId).find('[name=chartUseful][value=no]').click()
    page.getGraphFeedbackForm(chartId).find('[name=noComments]').type('I’m confused')
    page.getGraphFeedbackForm(chartId).submit()
    ...
    page.getGraphFeedback(chartId).parent().should('have.attr', 'open')
    // "no" should already be checked
    page.getGraphFeedbackForm(chartId).find('[name=chartUseful][value=no]').should('have.attr', 'checked')
    // typed comments should still be there
    page.getGraphFeedbackForm(chartId).find('[name=noComments]').should('have.value', 'I’m confused')
    // same error message should show
    page.getGraphFeedbackForm(chartId).find('p.govuk-error-message').contains('Select a reason for your answer')
  })
}
