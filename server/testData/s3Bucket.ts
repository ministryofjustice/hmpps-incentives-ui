...
export function mockAppS3ClientResponse(s3Client: jest.Mocked<S3Client>, tableType: TableType) {
  ...
  s3Client.getObject.mockReturnValue(
    fs.readFile(path.resolve(__dirname, `./s3Bucket/${tableType}/2022-03-13.json`), { encoding: 'utf8' })
  )
}
...
export function mockSdkS3ClientReponse(
  ...
  send.mockImplementation(async command => {
    ...
    if (command instanceof GetObjectCommand) {
      const stream = createReadStream(path.resolve(__dirname, `../testData/s3Bucket/${tableType}/2022-03-13.json`))
